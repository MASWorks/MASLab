...
from .agents import IO_MATH,FinalDecision
...
class GPTswarm_MATH(MAS):
    def __init__(self, general_config, method_config_name="config"):
        ...
        self.organize()
        inference_execute_token_stats[self.model_name_execute] = {"num_llm_calls": 0, "prompt_tokens": 0, "completion_tokens": 0}
    ...
    def _generate_graph(self,edge_mask: torch.Tensor) -> Tuple[torch.Tensor]:
        _graph = deepcopy(self)
        for _, (potential_connection, is_edge) in enumerate(zip(self.potential_connections, edge_mask)):
            out_node = _graph.nodes.get(potential_connection[0])
            in_node = _graph.nodes.get(potential_connection[1])
    ...
    def optimizing(self,val_dataset,batch_size: int = 4) -> torch.Tensor:
        ...
        if optimized_path.exists():
            ...
            return
        print(colored("Optimizing swarm on MATHDataset split dev...","light_yellow"))
        ...
        for i_iter in range(self.num_iterations):
            ...
            for _, record in zip(range(batch_size), loader):
                ...
                inference_execute_token_stats[self.model_name_execute]["num_llm_calls"] += graph.token_stats[self.model_name_execute]["num_llm_calls"]
                inference_execute_token_stats[self.model_name_execute]["prompt_tokens"] += graph.token_stats[self.model_name_execute]["prompt_tokens"]
                inference_execute_token_stats[self.model_name_execute]["completion_tokens"] += graph.token_stats[self.model_name_execute]["completion_tokens"]
            ...
            for raw_answer, log_prob, correct_answer in zip(raw_answers, log_probs, correct_answers):
                ...
                is_correct = grade_answer(answer,correct_answer)
        ...
        with open(token_path,"w") as f:
            json.dump(inference_execute_token_stats, f, indent=4)
