...
LLM_debate = {
    ...
    debate_agents = [LLMAgentBase(['thinking', 'answer'], 'Debate Agent', temperature=0.8, role=role) for role in ['Math Professor', 'Calculus Tutor', 'Math Enthusiast','Data Science Mentor']]
...
Role_Assignment = {"thought": "Similar to Auto-GPT and expert prompting, we can use dynamic control flow in the design to let the agent decide what expert we should use.",
        ...
        # Instruction for step-by-step reasoning
        cot_instruction = "Please think step by step and then solve the task."
        expert_agents = [LLMAgentBase(['thinking', 'answer'], 'Expert Agent', role=role) for role in ['Math Professor', ' Calculus Tutor', 'Math Enthusiast', 'Helpful Assistant','Data Science Mentor']]

        # Instruction for routing the task to the appropriate expert
        routing_instruction = "Given the task, please choose an Expert to answer the question. Choose from: Math Professor, Calculus Tutor, Math Enthusiast, Data Science Mentor."
        ...
        if 'professor' in choice.content.lower():
            expert_id = 0
        elif 'tutor' in choice.content.lower():
            expert_id = 1
        elif 'enthusiast' in choice.content.lower():
            expert_id = 2
        elif 'mentor' in choice.content.lower():
            expert_id = 3
        else:
            expert_id = 4 # Default to helpful assistant
...
base = """# Overview
You are an expert machine learning researcher testing various agentic systems. Your objective is to design building blocks such as prompts and control flows within these systems to solve complex tasks. Your aim is to design an optimal agent performing well on the MATH Benchmark which evaluates mathematical problem-solving abilities.
