...
from .adas_utils import random_id, bootstrap_confidence_interval, load_drop, drop_metric
...
from .prompt.drop_prompt import get_init_archive, get_prompt, get_reflexion_prompt
...
class ADAS_DROP(MAS):
    def __init__(self, general_config, method_config_name):
        super().__init__(general_config)
        # set the meta model and execute model for optimizing mode
        self.model_api_config = general_config["model_api_config"]
        self.optimize_execute_model = general_config['optimize_execute_model_name']
        self.optimize_meta_model = general_config['optimize_meta_model_name']
    ...
    def evaluate_forward_fn(self, forward_str, data_path, searching_mode=True):
    ...
    # modified from https://github.com/luchris429/DiscoPOP/blob/main/scripts/launch_evo.py
        ...
        examples = load_drop(data_path)[1:-1]  # first one and the last one is for few-shot examples
    ...
    def optimizing(self, val_dataset):
        ...
        for solution in archive:
            ...
            try:    
                acc_list = self.evaluate_forward_fn(solution["code"],data_path=val_dataset)
            except Exception as e:
        ...
        for n in range(start, self.n_generation):
            ...
            for _ in range(self.debug_max):
                try:
                    acc_list = self.evaluate_forward_fn(next_solution["code"], data_path=val_dataset)
    ...
    def inference(self, query):
        ...
        base_url = self.execute_model_dict['model_url']
        self.max_workers = self.model_api_config[execute_model]["max_workers"]
        print(f'execute model: {execute_model}')
